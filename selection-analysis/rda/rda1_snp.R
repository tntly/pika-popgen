# --------------------------- #
# SNP Outlier Detection from RDA – Analysis 1 of 2
# --------------------------- #
# This script identifies SNPs with extreme loadings along the first RDA axis
# from the first RDA model, which used the environmental variables:
# - ppt_sum
# - vpdmin_min
# - elev
#
...
# - pika_rda_1.Rdata (contains pika_rda and env_data_lfmm_U)
# --------------------------- #
...
# Get SNPs with loading beyond +-2.3 SDs
pika_rda_outliers <- identify_outliers(x = pika_rda_loadings[, 1], z = 2.3)
print("Number of outlier SNPs for ppt_sum + vpdmin_min + elev + Condition(V1 + V2):")
...
colnames(corr_matrix) <- c("ppt_sum", "vpdmin_min", "elev")
...
write.table(pika_rda_outliers_df,
            file = "rda-results/pika_rda_outliers_1.txt",
            sep = "\t", row.names = FALSE, quote = FALSE)
print("Saved: pika_rda_outliers_1.txt")
